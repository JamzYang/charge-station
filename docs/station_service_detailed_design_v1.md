# 场站域服务 (Charge Station) 详细开发设计文档 V1.0
...
- [场站域服务 (Charge Station) 详细开发设计文档 V1.0](#场站域服务-charge-station-详细开发设计文档-v10)
  - [目录](#目录)
  - [1. 文档概述 (Overview)](#1-文档概述-overview)
    - [1.1 版本历史](#11-版本历史)
    - [1.2 项目背景与目标](#12-项目背景与目标)
    - [1.3 术语表](#13-术语表)
  - [2. 总体设计 (High-Level Design)](#2-总体设计-high-level-design)
    - [2.1 系统交互图](#21-系统交互图)
    - [2.2 技术选型](#22-技术选型)
  - [| **Gradle**          |      | 依赖管理               | 依赖管理 |](#-gradle-----------------依赖管理----------------依赖管理-)
  - [3. 详细设计 (Detailed Design)](#3-详细设计-detailed-design)
    - [3.1 代码分层与结构](#31-代码分层与结构)
      - [3.1.1 推荐包结构](#311-推荐包结构)
      - [3.1.2 各层职责说明](#312-各层职责说明)
    - [3.2 关键类与方法设计](#32-关键类与方法设计)
      - [3.2.1 StationController (接口层)](#321-stationcontroller-接口层)
      - [3.2.2 DeviceControlApplicationService (应用层)](#322-devicecontrolapplicationservice-应用层)
      - [3.2.3 GatewayCommandRouter (基础设施层)](#323-gatewaycommandrouter-基础设施层)
      - [3.2.4 DeviceStatusEventHandler (应用层事件处理器)](#324-devicestatuseventhandler-应用层事件处理器)
      - [3.2.5 CommandResponseEventHandler (应用层事件处理器)](#325-commandresponseeventhandler-应用层事件处理器)
      - [3.2.6 共享值对象设计 (DeviceStatus)](#326-共享值对象设计-devicestatus)
    - [3.3 API接口设计](#33-api接口设计)
      - [3.3.1 充电站管理与控制API](#331-充电站管理与控制api)
    - [3.4 数据库设计](#34-数据库设计)
      - [3.4.1 充电站表 (stations)](#341-充电站表-stations)
      - [3.4.2 发件箱表 (outbox\_events)](#342-发件箱表-outbox_events)
    - [3.5 缓存设计](#35-缓存设计)
      - [3.5.1 缓存策略](#351-缓存策略)
      - [3.5.2 共享常量定义](#352-共享常量定义)
    - [3.6 异常处理](#36-异常处理)
      - [3.6.1 异常分类与映射](#361-异常分类与映射)
      - [3.6.2 全局异常处理器](#362-全局异常处理器)
  - [4. DDD在Spring Boot中的最佳实践](#4-ddd在spring-boot中的最佳实践)
    - [4.1 事件处理与发布最佳实践（事务性发件箱模式）](#41-事件处理与发布最佳实践事务性发件箱模式)
      - [4.1.1 整体流程](#411-整体流程)
      - [4.1.2 事件中继器实现 (OutboxEventRelay)](#412-事件中继器实现-outboxeventrelay)
      - [4.2.4 集成事件定义](#424-集成事件定义)
  - [5. 样板代码与最佳实践](#5-样板代码与最佳实践)
    - [5.1 Controller层样板代码](#51-controller层样板代码)
    - [5.2 DTO样板代码 (使用Record)](#52-dto样板代码-使用record)
  - [5. 分布式协调与缓存高级实践](#5-分布式协调与缓存高级实践)
    - [5.1 使用Redisson实现分布式锁](#51-使用redisson实现分布式锁)
  - [6. 测试要点](#6-测试要点)
    - [6.1 单元测试清单](#61-单元测试清单)
    - [6.2 集成测试清单](#62-集成测试清单)
  - [7. 总结](#7-总结)
